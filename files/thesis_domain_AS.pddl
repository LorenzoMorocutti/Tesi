...
(:predicates 
        ...
        (happy ?x1 - id ?ag1 - agent ?x2 - id)
        (sad ?x1 - id ?ag1 - agent ?x2 - id)
        (angry ?x1 - id ?ag1 - agent ?x2 - id)
        (ashamed ?x1 - id ?ag1 - agent ?x2 - id)
        (proud ?x1 - id ?ag1 - agent ?x2 - id)
        (ok1)
        (ok2)
        (ok3)
        (ok4)
...
(:derived (happy ?x2 - id ?ag2 - agent ?x4 - id)
        (exists(?x1 ?x3 - id ?ag1 ?ag - agent)
            (and
                (blamed ?x1 ?ag1 ?x3 ?ag)
                (Know ?x2 ?ag2 ?x1)
                (isEnd ?x4)
...
;isteric-envious
(:derived (angry ?x2 - id ?ag2 - agent ?x3 - id)
        (exists(?x1 - id ?ag1 - agent) 
            (and
                (happy ?x1 ?ag1 ?x3)
                (Know ?x2 ?ag2 ?x1)
...
;-- The action of "blaming" and "complimenting" for something are similar to insulting and praising. The only difference is that agents are "blamed" or "complimented" for something, for instance ?ag1 can be blamed or complimented for the fact that the ball is in the box (to keep the model simple, we blame or compliment agents for facts and not for actions: we blame or compliment ?ag1 for the fact that the ball is in the box, even if ?ag1 is not the one that put the ball in the box).
...
;-- Sixth test: Sally is feeling both happy about something and angry 
;-- because Anne has been insulted, while Anne is sad about something
